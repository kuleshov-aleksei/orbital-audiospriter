...
import { detectCapabilities, REQUIRED_ENCODERS, REQUIRED_FILTERS } from "@/services/capabilities"
...
<template>
  <div class="mx-auto flex max-w-5xl flex-col gap-6 px-6 py-8">
    <div>
      <h2 class="text-xl font-semibold text-zinc-100">Phase 1 spike — codec/filter capability matrix</h2>
      <p class="mt-1 max-w-3xl text-sm text-zinc-400">
        Verifies the published <code class="text-violet-300">@ffmpeg/core-st</code> build supports what the spec needs:
        MP3 (libmp3lame), OGG (libvorbis/libopus), M4A (aac/libfdk_aac) and the <code class="text-violet-300">loudnorm</code> filter.
        No result here is fatal — each has a documented fallback.
      </p>
    </div>
    ...
    <section class="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5">
      ...
      <div class="mt-3 flex gap-2">
        ...
        >
          {{ capsStatus === "running" ? "Checking…" : caps.value ? "Re-check" : "Check capabilities" }}
        </button>
      </div>
      ...
      <div v-if="verdict" class="mt-4 grid gap-6 md:grid-cols-2">
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-zinc-800">
            ...
            <tr>
              ...
              <td class="py-2 font-mono text-xs">{{ verdict.loudnorm ? "available" : "fallback: JS EBU R128" }}</td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="text-sm leading-relaxed text-zinc-400">
          <p v-if="caps">
            Core: <span class="text-zinc-200">{{ (caps.core.sizeBytes / 1024 / 1024).toFixed(1) }} MB</span> wasm,
            loaded in <span class="text-zinc-200">{{ caps.core.loadTimeMs }} ms</span>.
          </p>
          <p class="mt-2 text-xs text-zinc-500">
            Relevant encoders found: {{ REQUIRED_ENCODERS.filter((e) => caps?.encoders.includes(e)).join(", ") || "none" }}
          </p>
          <p class="mt-1 text-xs text-zinc-500">
            Relevant filters found: {{ REQUIRED_FILTERS.filter((f) => caps?.filters.includes(f)).join(", ") || "none" }}
          </p>
        </div>
      </div>
    </section>
